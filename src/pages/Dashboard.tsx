      ...
      {metrics.length === 0 ? (
        ...
        <div className="space-y-6">
          {metrics.map((metric) => {
            ...
            const cropHedges = hedgeData[metric.crop_type];
            ...
            const percentSold = totalProduced > 0
              ...
              : 0;
            const percentHedged = totalProduced > 0
              ? (cropHedges.total_bushels_hedged / totalProduced) * 100
              : 0;
            const showSalesRow = cropSales.sale_count > 0 || totalProduced > 0;
            const showHedgeRow = cropHedges.hedge_count > 0;
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        <div>
                          ...
                          <p className="text-lg font-semibold text-gray-900">
                            {cropSales.total_bushels_sold > 0 ? formatNumber(cropSales.total_bushels_sold, 0) : '0'}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600">Avg Sale Price</p>
                          <p className="text-lg font-semibold text-gray-900">
                            {cropSales.weighted_avg_price > 0 ? formatCurrency(cropSales.weighted_avg_price) : 'N/A'}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600">Total Sales Revenue</p>
                          <p className="text-lg font-semibold text-green-700">
                            {cropSales.total_sales_revenue > 0 ? formatCurrency(cropSales.total_sales_revenue) : '$0.00'}
                          </p>
                        </div>
                        <div>
                          ...
                          <p className={`text-lg font-semibold ${bushelsRemaining > 0 ? 'text-amber-600' : 'text-gray-900'}`}>
                            {totalProduced > 0 ? formatNumber(Math.max(bushelsRemaining, 0), 0) : 'N/A'}
                          </p>
                        </div>
                        <div>
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        <div>
                          ...
                          <p className="text-lg font-semibold text-gray-900">
                            {formatNumber(cropHedges.total_bushels_hedged, 0)}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600">Avg Net Price</p>
                          <p className="text-lg font-semibold text-blue-700">
                            {formatCurrency(cropHedges.weighted_avg_net_price)}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600">Avg Futures / Basis</p>
                          <p className="text-lg font-semibold text-gray-900">
                            {formatCurrency(cropHedges.weighted_avg_futures)} / {cropHedges.weighted_avg_basis >= 0 ? '+' : ''}{formatCurrency(cropHedges.weighted_avg_basis)}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600">Locked-In Value</p>
                          <p className="text-lg font-semibold text-blue-700">
                            {formatCurrency(cropHedges.total_bushels_hedged * cropHedges.weighted_avg_net_price)}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600">% Hedged</p>
                          <p className="text-lg font-semibold text-gray-900">
                            {totalProduced > 0 ? `${Math.min(percentHedged, 100).toFixed(1)}%` : `${cropHedges.total_bushels_hedged.toLocaleString('en-US', { maximumFractionDigits: 0 })} bu`}
                          </p>
                          {totalProduced > 0 && (
                            <div className="mt-1 w-full bg-gray-200 rounded-full h-1.5">
                              <div
                                className="bg-blue-500 h-1.5 rounded-full transition-all"
                                style={{ width: `${Math.min(percentHedged, 100)}%` }}
                              />
                            </div>
                          )}
                        </div>
